...
class ImageUtils {
    ...
    localGrayscaleArray(img) {
        return __awaiter(this, void 0, void 0, function* () {
            const imgForResize = yield loadImage(img);
...
function performCircles2(image, radius, results, binMergeCount) {
    return __awaiter(this, void 0, void 0, function* () {
        const resultList = [];
...
function process(fileIndex) {
    return __awaiter(this, void 0, void 0, function* () {
        const eleSpinner = document.querySelector('.loader');
        eleSpinner === null || eleSpinner === void 0 ? void 0 : eleSpinner.removeAttribute('style');
        ...
        try {
            yield moravecProcessor.process(selectedImage1);
            yield moravecProcessor2.process(selectedImage2);
            yield moravecProcessor.process2();
            yield moravecProcessor2.process2();
        ...
        do {
            ...
            do {
                ...
                const hrlist = yield performCircles2(image, circleSize, morResults, binMergeCount);
                const hrlist2 = yield performCircles2(image2, circleSize, morResults2, binMergeCount);
                binMergeCount++;
...
class MoravecProcessor {
    ...
    process(imageSrc) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            this.mGrayscaleData = yield imageUtils.localGrayscaleArray(imageSrc);
    ...
    getShannonEntropyImage(x, y, endX, endY) {
        ...
        if (y < 0)
            y = 0;
        if (endX >= this.mGrayscaleData.length)
            endX = this.mGrayscaleData.length - 1;
        if (endY >= this.mGrayscaleData[0].length)
            endY = this.mGrayscaleData[0].length - 1;
        const values = [];
        let n = 0;
        const occ = {};
        for (let i = y; i < endY; i++) {
            for (let j = x; j < endX; j++) {
                ...
                if (!values.includes(String(pixel)))
                    values.push(String(pixel));
    ...
    processWithMarkup(imageSrc) {
        return __awaiter(this, void 0, void 0, function* () {
            const eleSpinner = document.querySelector('.loader');
            eleSpinner.removeAttribute('style');
            yield this.process(imageSrc);
            const imageUtils = new ImageUtils();
            const newImage = yield imageUtils.markupGrayscaleWithResults(this.mGrayscaleData, this.mResults);
            return newImage;
        });
    }
    getResults() {
...
TimeData.imageLoad = 0;
TimeData.moravec = 0;
TimeData.matching = 0;
TimeData.matchingTimes = [];
TimeData.moravecTimes = [];
TimeData.distinctivenessAlignmentTimes = [];
TimeData.binDistanceUsage = new Array(6).fill(0);
TimeData.iterationCountUsage = new Array(10).fill(0);
TimeData.iterationCountUsageMap = new Map();
TimeData.featureMatches = 0;
