...
document.getElementById('limitBestMatches').addEventListener('change', function () {
    limitToBestMatches = (this as HTMLInputElement).checked;
...
class CircleResult {
    public hist: number[];
    public innerHist: number[];
    public centerHist: number[];
...
class FeatureMatch {
    ...
    public x2: number;
    public y2: number;
    public roughBinDistance: number;
    public rotationArrayIndex: number;
...
class HistResult {
    ...
    public x: number;
    public y: number;
    public distinctiveness: number;
    public minDistinctiveness: number;
    public maxDistinctiveness: number;
    public longestSequence: number;
    public distances: number[];
    public distances2: number[];
...
class HistResultList {
    public histResults: HistResult[];
    public distinctivenessAverage: number;
...
class LocalMaximumList {
    ...
    public localFeatureCount: number;
    public results: PriorityQueue;
...
async function performCircles2(image, radius, results, binMergeCount) {
    const resultList: HistResultList[] = [];
...
async function process(fileIndex) {
    ...
    let featureMatches: FeatureMatch[] = [];
    ...
    newImage3.toBlob(blob => {
        ...
        img.onload = () => {
            ...
            const eleSpinner: HTMLSpanElement = document.querySelector('.loader') as HTMLSpanElement;
...
class MoravecProcessor {
    public mBaseThreshold: number;
    public mLocalFeatureCount: number;
    public mLocalFeaturePercentage: number;

    public mResults: number[];
    public mGrayscaleData: number[][];
    ...
    process2() {
        ...
        const localMaximumLists: LocalMaximumList[] = [];
        ...
        for (let y = 1; y < height - 1; y++) {
            for (let x = 1; x < width - 1; x++) {
                ...
                if (ssd > this.mBaseThreshold) {
                    ...
                    let matchingMaximumList: LocalMaximumList | null = null;
...
function handleFileSelect(event) {
    ...
    if (!file) {
        ...
        const btnEle: HTMLButtonElement = document.getElementById('runBtn') as HTMLButtonElement;
